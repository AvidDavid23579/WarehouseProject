...
def edge_normals(polygon: list[tuple[float, float]]):
    n = len(polygon)
    for i in range(n):
        x1, y1 = polygon[i]
        x2, y2 = polygon[(i + 1) % n]
...
def update_obb_vertices(pose: NDArray[np.float32], vertices: NDArray[np.float32], length: float, width: float):
    # Module constants
    HL = length * 0.5
    HW = width * 0.5
...
def obb_aabb_distance(
    ...
    # Compute R^T p into px/py. Use t0 to t3 as scratch before overwriting.
